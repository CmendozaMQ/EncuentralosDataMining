...
def getFacebookPageFeedData(page_id, access_token, num_statuses, since_date, until_date):
    ...
    fields = "/?fields=message,link,created_time,type,name,id," + \
            ...
            ".limit(0).summary(true),from&until=%s&since=%s" % (until_date, since_date)
...
def processFacebookPageFeedStatus(status, access_token):
    ...
    status_link = '' if 'link' not in status.keys() else \
        status['link'] #unicode_normalize(status['link'])
...
def scrapeFacebookPageFeedStatus(page_id, access_token, since_date, until_date):
    with open_csv_w('%s_facebook_statuses.csv' % page_id) as file:
        ...
        statuses = getFacebookPageFeedData(page_id, access_token, 100, since_date, until_date)
